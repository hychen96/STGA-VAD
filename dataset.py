...
class Train_Dataset(Dataset):
    ...
    def load_feat(self):
        ...
        for line in lines:
            ...
            if label == 0:
                # f = np.array(h5[key + '.npy'][:, 0:5, :])
                # self.norm_feats.append(f.reshape((-1, f.shape[-1])))
                self.norm_feats.append(h5[key + '.npy'][:])
                # self.norm_feats.append(h5[key + '.npy'][:, 8, :])  #for ucf crime
                if self.pseudo_labels != None:
                    self.norm_keys.append(key + '.npy')
            else:
                # f = np.array(h5[key + '.npy'][:, 0:5, :])
                # self.abnorm_feats.append(f.reshape((-1, f.shape[-1])))
                self.abnorm_feats.append(h5[key + '.npy'][:])
                # self.abnorm_feats.append(h5[key + '.npy'][:, 8, :])   #for ucf crime
                if self.pseudo_labels != None:
...
def Test_Dataset(txt_path,mask_dir,h5_file,norm):
    ...
    for line in lines:
        ...
        feat = h5[line_split[0] + '.npy'][:]
        # feat = h5[line_split[0] + '.npy'][:, 8, :]  #for ucf_crime
        # f = np.array(h5[line_split[0] + '.npy'][:, 0:5, :])
        # feat = f.reshape((-1, f.shape[-1]))
        if norm==2:
